...
addCommandHandler( 'hpanel', function( player )
  if isObjectInACLGroup( 'user.'..getAccountName( getPlayerAccount( player ) ), aclGetGroup( 'House' ) ) or hasObjectPermissionTo( player, 'function.banPlayer', false ) then
    ...
    outputChatBox( '#FF9900 * #FFFFFF[ #0099FFHouse System #FFFFFF] :#FF0000 Access denied for this command!', player, 255, 36, 51,true );
  end;
end );
...
addEventHandler( 'onPlayerAttemptCreateHouse', root, function( rt )
  ...
  outputChatBox( '#FF9900 * #FFFFFF[ #0099FFHouse System #FFFFFF] :#FF0000 تهانينا تم تكوين بيت جديد', client, 255, 255, 0,true );
end );
...
function createHouse( add, ID, owner, key, eX, eY, eZ, etX, etY, etZ, exX, exY, exZ, extX, extY, extZ, int, dim, cost )
    ...
    if getElementType( player ) == 'player' and not getPedOccupiedVehicle( player ) then
      if not getElementData( player, 'HP_Opened' ) then
        if not isGuestAccount( getPlayerAccount( player ) ) then
          ...
          outputChatBox( '#FF9900 * #FFFFFF[ #0099FFHouse System #FFFFFF] :#FF0000 يجب عليك تسجيل دخولك', player, 255, 51, 36,true);
        end;
      end;
    end;
  end );
  ...
  local m_Exit = createMarker( exX, exY, exZ - 1, 'cylinder', 1.25, 255, 255, 0, 255 );
...
function onPlayerHouseMarkerHit( player, mrk, cursor )
  ...
  if isGuestAccount( acc ) then
    outputChatBox( '#FF9900 * #FFFFFF[ #0099FFHouse System #FFFFFF] :#FF0000 يجب عليك التسجيل لشراء هذا البيت', player, 255, 51, 36,true);
    setElementData( player, 'mrk_in', nil );
    setElementFrozen( player, false );
    return false;
  end;
  local tts = { [1] = true, [2] = false, [3] = false, [4] = true, [5] = false, [6] = false };
  if isObjectInACLGroup( 'user.'..getAccountName( acc ), aclGetGroup( 'House' ) ) or hasObjectPermissionTo( player, 'function.banPlayer', false ) then
    tts[6] = true;
  end;
...
addEventHandler( 'HOUSE_Buy', root, function( cost, key )
  ...
  if houseCounter >= tonumber( get( 'playerHouseCounter' ) ) then
    outputChatBox( '#FF9900 * #FFFFFF[ #0099FFHouse System #FFFFFF] :#FF0000 منزلا في نفس الوقت [ '..get( 'playerHouseCounter' )..' ] لا تستطيع شراء', client, 255, 51, 36, true );
    setElementFrozen( client, false );
    return false;
  end;
  if getPlayerMoney( client ) >= tonumber( cost ) then
    outputChatBox( '#FF9900 * #FFFFFF[ #0099FFHouse System #FFFFFF] :#FF0000 الرقم السري الرجاء عد نسيانه '..key, client, 255, 51, 36, true );
    outputChatBox( '#FF9900 * #FFFFFF[ #0099FFHouse System #FFFFFF] :#FF0000 تهانينا تم شراء البيت بنجاح', client, 255, 255, 0,true);
    ...
    outputChatBox( '#FF9900 * #FFFFFF[ #0099FFHouse System #FFFFFF] :#FF0000 ليس لديك مال كافي لشراء البيت', client, 255, 51, 36,true);
...
addEventHandler( 'HOUSE_ChangeOwner', root, function( name )
  ...
  setElementData( mrk, 'HS_INFO', { t[1], t[2], t[3], t[4], t[5], t[6], t[7], t[8], t[9] } );
  setTimer( onPlayerHouseMarkerHit, 50, 1, client, mrk, false );
  outputChatBox( '#FF9900 * #FFFFFF[ #0099FFHouse System #FFFFFF] :#FF0000 مبروك #FFFFFF[ #FF9900'..getPlayerName( client )..' #FFFFFF] #FF0000 هذا اللاعب اعطاك بيته!', getPlayerFromName( name ), 0, 255, 0, true );
end );
...
addEventHandler( 'HOUSE_Destroy', root, function()
  ...
  outputChatBox( '#FF9900 * #FFFFFF[ #0099FFHouse System #FFFFFF] :#FF0000 تم #'..getElementData( source, 'mrk_in' )..' حذف هذا البيت', source, 255, 255, 0,true);
