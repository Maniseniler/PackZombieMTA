...
function onMTstart()
	outputDebugString("[Missions]Transporter By DeaD | Started")
	--
	setUpMarkers()
	--
...
function setUpMarkers()
	...
	if ( transPosz and type(transPosz) == "table" ) then
						...
						function onHit(hitter)
							...
							if not tFlood[hitter] == true then
								if not missionOn then
									...
									outputChatBox("#FF9100[#FFEE00 Transporter Mission #FF9100]#FFFFFF :#00EAFF تم تشغيل مود التوصيل",hitter,255,0,0,true)
								end
							else
									outputChatBox("#FF9100[#FFEE00 Transporter Mission #FF9100]#FFFFFF :#00EAFF لا يمكنك العمل هنا لانك كنت تعمل منذ قليل ارجع بعد دقائق",hitter,255,0,0,true)
							end
							end
						end 
		...
		outputChatBox("#FF9100[#FFEE00 Transporter Mission #FF9100]#FFFFFF :#00EAFF يمكنك العمل في مهمة التوصيل الأن",root,255,0,0,true)
		missionOn = false
end
...
function startTransporterMission(plr) -- starting new transporter mission
	if plr then
		...
		outputChatBox("#FF9100[#FFEE00 Transporter Mission #FF9100]#FFFFFF :#00EAFF تم اعطائك سيارة لايصالها",root,255,0,0,true)
		--
		easytextout(plr,05551, "-| مهمة الايصال بدأت |-", 5 * 1000, 0.5, 0.7, 0, 255, 255, 255,2)
		--
		...
		setTimer(function() fadeCamera(plr,true,2)
			...
			firstX,firstY,firstZ = x,y,z
				...
				if transVeh and transPed then
					...
					setTimer(function()
							easytextout(plr,05551, "-| اركب في السيارة |-", 1000 * 1000, 0.5, 0.7, 0, 255, 255, 255,2)
										...
										vehArrow = createMarker(vx,vy,vz+2,"arrow",1,255,255,0)
											if vehArrow then
												function onVehEnter(enterer,seat)
													...
													--
													easytextout(plr,05551, "يا سواق روح للعلامة {D}", 5 * 1000, 0.5, 0.8, 255, 0, 0, 255,2)
...
if Player then
	...
	if Won == true then -- If he won the mission
		...
		--
		prize = tonumber(theTransporterPrize) or 20000
		givePlayerMoney(Player,prize)
		easytextout(Player,05551, "$"..tonumber(theTransporterPrize).." تم ايصال الرئيس بنجاح و كان مربوحك", 8 * 1000, 0.5, 0.7, 0, 255, 0, 255,2)
		outputChatBox("#FF9100[#FFEE00 Transporter Mission #FF9100]#FFFFFF :#49FA60 ["..getPlayerName(Player).."#49FA60] #49F3FC لقد اوصل الرئيس بنجاح #FF6363( #1EFF00"..tonumber(theTransporterPrize).." #FF6363#1EFF00$) #00FFCC وربحت ",root,0,255,255,true)
		--
		triggerClientEvent(Player,"missionDone",Player,true)
	else
		triggerClientEvent(Player,"missionDone",Player,false)
		easytextout(Player,05551, " -| لقد فشلت في مهمة التوصيل |-", 8 * 1000, 0.5, 0.7, 255, 0, 0, 255,2)
	end
...
function sortTimerShit(plr,timer,time) -- to sort timer's shit ..
	if timer and time then
		...
		timerShitTimer = setTimer(function(plr)
				...
				fullTime = m..":"..s
				textItemSetText(timer,""..tostring(fullTime)..": الوقت المتبقي")
